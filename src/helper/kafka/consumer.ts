import { Consumer, EachMessageHandler, EachMessagePayload, Kafka, KafkaConfig, PartitionAssigners } from "kafkajs"
...
class KafkaConsumer {
    ...
    private topics: Record<string, Function> = {}   
    private groupId: any
    private concurrencyNumber: number
    private health: boolean

    constructor(groupId: any, concurrencyNumber = 1) {
        this.option = {
    ...
    registerTopic(topic: string, handler: Function) {
        ...
        try {
          ...
          await this.consumer.run({
            ...
            eachMessage: async ({ message, topic, partition }) => {
              ...
              if (result.err) {
                console.error(`Kafka handler error when handling topic ${topic}`, 'run', result.err);
              } else {                
                try {
